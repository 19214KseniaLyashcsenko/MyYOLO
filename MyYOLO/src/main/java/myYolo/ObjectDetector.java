...
import myYolo.classifier.YOLOClassifier;
import myYolo.model.Recognition;
import myYolo.util.GraphBuilder;
import myYolo.util.IOUtil;
import myYolo.util.ImageUtil;
import myYolo.util.ServiceException;
...
public class ObjectDetector {
    ...
    public ObjectDetector() {
        try {
            GRAPH_DEF = IOUtil.readAllBytesOrExit(Config.GRAPH_FILE);
            LABELS = IOUtil.readAllLinesOrExit(Config.LABEL_FILE);
        } catch (ServiceException ex) {
            LOGGER.error("Download graph file to run the program! \n");
        }
    }
